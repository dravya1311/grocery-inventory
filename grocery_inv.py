...
if not df.empty and calculate_kpis(df) is not None:
    ...
    if 'Catagory' in df.columns:
        fig1 = px.bar(
            df.groupby('Catagory')['Inventory_Value'].sum().reset_index(),
            x='Catagory', y='Inventory_Value', color='Catagory',
            title="Inventory Value by Category", text_auto=True
        )
        st.plotly_chart(fig1, use_container_width=True)

    # Chart 2: Top 10 Revenue Products
    if 'Product_Name' in df.columns:
        top_products = df.groupby('Product_Name')['Total_Revenue'].sum().nlargest(10).reset_index()
        fig2 = px.bar(top_products, x='Product_Name', y='Total_Revenue', title="Top 10 Products by Revenue")
        st.plotly_chart(fig2, use_container_width=True)

    # Chart 3: Expiring Soon Table
    st.subheader("🚨 Products Near Expiration")
    expiring = df[df['Days_to_Expire'] <= 30][
        ['Product_Name', 'Catagory', 'Supplier_Name', 'Stock_Quantity', 'Expiration_Date', 'Days_to_Expire']
    ].sort_values('Days_to_Expire')
    st.dataframe(expiring, use_container_width=True)

else:
    st.warning("⚠️ Please upload a valid 'Grocery_Inventory.csv' file to visualize the dashboard.")
